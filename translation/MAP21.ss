...
{
	//End Cinematic
	If(CheckInventory("ScriptHasDefeatedBlueBaron") > 0)
	{
...
Script 34 (void)
{
	If(GetActorZ(0) < 360.0)
	{
		...
		*/
		...
		Delay(4);
		ChangeLevel("Map22", 2, CHANGELEVEL_NOINTERMISSION, -1);
		Terminate;
	}
	Delay(4);
	Restart;
...
Script 40 (void)
{
	SectorDamage(250, 16, "Lava", "", DAMAGE_PLAYERS | DAMAGE_NONPLAYERS | DAMAGE_NO_ARMOR);
	Delay(16);
	Restart;
}
...
Script 118 (void)
{
	...
	{
	  spec118 = 1;
	  SetActorFlag(666, "INVULNERABLE", 1);
...
Script 1002 (void)
{
  If (GetActorZ(0) < 29.0 && CheckInventory("VineKey") > 0)
  {
	...
	Delay(55);
	Hudmessage(s:"Abhaoth:    Make. Them. Pay."; HUDMSG_FADEINOUT | HUDMSG_LOG, 100, CR_ORANGE, 1.5, 0.84, 1.5, 1.2, 0.5);
	PlaySound(0, "ShadowAbhaoth/WhisperAnger", CHAN_AUTO);
	Delay(70);
	Radius_Quake(1, 35, 0, 1, 0);
...
Script 108 (void)
{
	...
	SetActorFlag(666, "INVULNERABLE", 0);
...
Script 82 (void)
{
	...
	//SetActorFlag(666, "INVULNERABLE", 1); //in script 118
	FadeTo(255, 100, 180, 1.0, 3.0);
	Delay(1*35);
	SetActorFlag(0, "NODAMAGE", 1);
	Delay(2*35);
...
Script 84 (void)
{
	...
	Thing_Destroy(852, 0, 0); //kill PsiDemons so they don't see spam
    ...
    ChangeCamera(1907, 0, 0);
	Radius_Quake2(1907, 1, 9*35, 0, 64, "");
	Thing_Activate(752);
	//ChangeSky("Sky11", "Sky11");
	//SetSkyScrollSpeed(1, 0.002365);
    Delay(5*35);
	FadeTo(0, 0, 0, 1.0, 4.0);
	Delay(4*35);
	Thing_Remove(752);
	...
	Hudmessage(s:"Breathelast"; HUDMSG_FADEINOUT | HUDMSG_LOG, 102, CR_WHITE, 1.5, 0.18, 7.0, 1.0, 2.0);
	Hudmessage(s:"Alexander Brandon"; HUDMSG_FADEINOUT | HUDMSG_LOG, 103, CR_WHITE, 1.5, 0.20, 7.0, 1.0, 2.0);
	...
	Hudmessage(s:"Ariah"; HUDMSG_FADEINOUT | HUDMSG_LOG, 107, CR_WHITE, 1.5, 0.28, 7.0, 1.0, 2.0);
	...
	{
		Hudmessage(s:"Under the Curator's care, the Disk worked to reinvigorate the dying crystal formations."; HUDMSG_FADEINOUT | HUDMSG_LOG, 1, CR_WHITE, 1.5, 0.88, 9.0, 2.0, 2.0);
	}
	else
	{
	  Thing_Remove(597);
      Hudmessage(s:"The Disk was returned just in time to reinvigorate the dying energy crystal formations."; HUDMSG_FADEINOUT | HUDMSG_LOG, 1, CR_WHITE, 1.5, 0.88, 9.0, 2.0, 2.0);
	}
	Delay(4*35);
	...
	Hudmessage(s:"\c[Red]RawbmGM\c-     \c[Yellow]White Sarcophagus\c-     \c[Purple]Anri_Wanri\c-     Joseph Culbertson     Terrible Gyros     \c[Gold]Nick\c-"; HUDMSG_FADEINOUT | HUDMSG_LOG, 16, CR_UNTRANSLATED, 1.5, 0.44, 18.0, 1.0, 1.0);
...
Script 60 (void)
{
	...
	Hudmessage(s:"Loading..."; HUDMSG_FADEINOUT | HUDMSG_LOG, 100, CR_GRAY, 1.5, 0.86, 0.1, 0.1, 0.1);
	Delay(4);
	ChangeLevel("Map22", 4, CHANGELEVEL_NOINTERMISSION, -1);
}
